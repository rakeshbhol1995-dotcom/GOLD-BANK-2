...
// Dedicated Program ID Placeholder for Virtual Gold Anchor Protocol
// NOTE: Run `anchor keys sync` prior to mainnet deployment to bind actual program key pair
...
pub mod immortal_gold_protocol {
    ...
    pub fn initialize(ctx: Context<Initialize>, signers: Vec<Pubkey>, threshold: u8) -> Result<()> {
        ...
        config.unallocated_dividends       = 0;
    ...
    pub fn inject_external_yield(ctx: Context<InjectExternalYield>, amount: u64) -> Result<()> {
        ...
        // If supply == 0, hold in unallocated_dividends and distribute when first buy happens
        if config.total_supply > 0 {
            let total_to_distribute = (amount as u128)
                .checked_add(config.unallocated_dividends).ok_or(VirtualGoldError::MathOverflow)?;
            let add = safe_div_u128(
                total_to_distribute.checked_mul(DIVIDEND_PRECISION).ok_or(VirtualGoldError::MathOverflow)?,
            ...
            // No holders yet — park yield until first buyer arrives
            config.unallocated_dividends = config.unallocated_dividends
                .checked_add(amount as u128).ok_or(VirtualGoldError::MathOverflow)?;
        }
    ...
    pub fn buy(ctx: Context<BuyTokens>, amount_to_buy: u64, max_cost_limit: u64) -> Result<()> {
        ...
        if bd.dividend_fee > 0 {
            ...
            if config.total_supply > 0 {
                // Include any unallocated yield (injected before first buyer)
                let total_to_distribute = (bd.dividend_fee as u128)
                    .checked_add(config.unallocated_dividends).ok_or(VirtualGoldError::MathOverflow)?;
                let add = safe_div_u128(
                    total_to_distribute.checked_mul(DIVIDEND_PRECISION).ok_or(VirtualGoldError::MathOverflow)?,
                    config.total_supply as u128,
        ...
        // ✅ Capture old balance BEFORE mint to settle pre-existing dividends correctly
        let old_bal = ctx.accounts.buyer_token_account.amount;

        let user = &mut ctx.accounts.user_account;
        if user.owner == Pubkey::default() { user.owner = ctx.accounts.buyer.key(); }
        user.last_buy_slot = Clock::get()?.slot;

        // ✅ Settle dividends earned on old_bal ONLY (before mint) — new tokens earn no past dividends
        if old_bal > 0 {
            let acc = (old_bal as u128).checked_mul(config.acc_dividend_per_share).ok_or(VirtualGoldError::MathOverflow)?;
        ...
        // If total_supply was 0 previously and unallocated_dividends exist, distribute now to first buy supply
        if config.unallocated_dividends > 0 && config.total_supply > 0 {
            let add = safe_div_u128(
                config.unallocated_dividends.checked_mul(DIVIDEND_PRECISION).ok_or(VirtualGoldError::MathOverflow)?,
                config.total_supply as u128,
            )?;
            config.acc_dividend_per_share = config.acc_dividend_per_share
                .checked_add(add).ok_or(VirtualGoldError::MathOverflow)?;
        ...
        // ✅ Set reward_debt based on new total balance — new tokens start fresh at current accDividendPerShare
        let new_bal = old_bal.checked_add(amount_to_buy).ok_or(VirtualGoldError::MathOverflow)?;
...
pub struct ProtocolConfig {
    ...
    pub unallocated_dividends: u128,          // 16 ← yield held when supply==0, distributed on first buy
}
impl ProtocolConfig {
